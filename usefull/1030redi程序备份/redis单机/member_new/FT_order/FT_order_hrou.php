...
$sgid=$gid-1;
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime_RB_H,TG_Dime_RB_H,MB_Dime_Rate_RB_H,TG_Dime_Rate_RB_H,MB_Ball,TG_Ball,overdue from `match_sports` where Type='FT' and `MID`='$sgid' and Cancel!=1 and Open=1 and source_type<>'BET' and $mb_team!=''";
...
if ($cou == 0 || $redisOverdue['overdue'] == '0') {
	...
	if($specialMem_row['status'] == '1'){
        ...
        $bettop=$league['CS'];
			...
			//$bettop=$league['VROU'];
			$GMAX_SINGLE=$GSINGLE_CREDIT>$league['VROU']?$league['VROU']:$GSINGLE_CREDIT;
			$GSINGLE_CREDIT=$GSINGLE_CREDIT>$league['VROU']?$league['VROU']:$GSINGLE_CREDIT;
			$bettop=$GSINGLE_CREDIT;
		}
	} 
...
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_hrb_finish.php" method="post" onSubmit="return false">
  <div class="ord ord_top">
    ...
    <div class="main">
      ...
      <div class="gametype">(滚球)&nbsp;大 / 小&nbsp;-&nbsp;上半场</div>
