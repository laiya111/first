...
//////////////////////////////////////////////////
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime_RB,TG_Dime_RB,MB_Dime_Rate_RB,TG_Dime_Rate_RB,MB_Ball,TG_Ball,overdue from `match_sports` where Type='FT' and `MID`=$gid and Cancel!=1 and Open=1 and source_type<>'BET' and $mb_team!=''";
...
if ($cou == 0 || $redisOverdue['overdue'] == '0') {
    if ($redisOverdue['overdue'] == '0')
        ...
        echo "<script>window.location.href = '/app/member/wager_finish.php?uid=$uid&langx=$langx&1=1&cou=$cou';</script>";
    exit();
} else{
	...
	if($specialMem_row['status'] == '1'){
		...
		if ($mmb_team[1]==$Special1){
			...
			$GMAX_SINGLE=$GSINGLE_CREDIT>$league['ROU']?$league['ROU']:$GSINGLE_CREDIT;
			$GSINGLE_CREDIT=$GSINGLE_CREDIT>$league['ROU']?$league['ROU']:$GSINGLE_CREDIT;
			//$bettop=$league['ROU'];
			$bettop=$GSINGLE_CREDIT;
		}
	}
...
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_rb_finish.php" method="post" onSubmit="return false">
  <div class="ord ord_top">
    ...
    ?>
      <div class="gametype">(滚球)&nbsp;大 / 小</div>
      <div class="leag"><?=$M_League?></div>
      <div class="teamName"><span class="tName"><?=$MB_Team?>&nbsp;&nbsp;vs.&nbsp;<?=$TG_Team?></span>&nbsp;&nbsp;<em class="bold">(<?=$inball?>)</em>
      </div>
      <p class="team"><em><span class="radio"><?=$M_Place?></span></em>&nbsp;&nbsp;@&nbsp;<strong class="light" id="ioradio_id"><?=$M_Rate?></strong></p>
