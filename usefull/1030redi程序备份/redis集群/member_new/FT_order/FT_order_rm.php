...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate_RB,TG_Win_Rate_RB,M_Flat_Rate_RB,MB_Ball,TG_Ball,overdue from `match_sports` where Type='FT' and `MID`=$gid and Cancel!=1 and Open=1 and source_type<>'BET' and $mb_team!=''";
...
if ($cou == 0 || $redisOverdue['overdue'] == '0') {
	...
	if($specialMem_row['status'] == '1'){
		...
		$GMAX_SINGLE=$GSINGLE_CREDIT>$league['RM']?$league['RM']:$GSINGLE_CREDIT;
		$GSINGLE_CREDIT=$GSINGLE_CREDIT>$league['RM']?$league['RM']:$GSINGLE_CREDIT;
	}
    //$bettop=$league['RM'];
...
<title></title>
<link href="/style/member/mem_order_ft<?=$css?>.css?v=20161216 " rel="stylesheet" type="text/css">
