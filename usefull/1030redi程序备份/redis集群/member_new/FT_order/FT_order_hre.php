...
$sgid=$gid-1;
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeHRB,M_LetB_RB_H,MB_LetB_Rate_RB_H,TG_LetB_Rate_RB_H,MB_Ball,TG_Ball,overdue from `match_sports` where Type='FT' and `MID`='$sgid' and cancel!=1 and Open=1 and source_type<>'BET' and mb_team!='' and mb_team_tw!=''";
...
if ($cou == 0 || $redisOverdue['overdue'] == '0') {
	...
	case "H":
		$M_Place=$MB_Team;
		$M_Rate=$MB_LetB_Rate_RB_H;
		$mtype='VRRH';
		break;
	case "C":
		$M_Place=$TG_Team;
		$M_Rate=$TG_LetB_Rate_RB_H;
		$mtype='VRRC';
		break;
	}
	...
	if($specialMem_row['status'] == '1'){
		...
		$bettop=$league['CS'];
			...
			$GMAX_SINGLE=$GSINGLE_CREDIT>$league['VRB']?$league['VRB']:$GSINGLE_CREDIT;
			$GSINGLE_CREDIT=$GSINGLE_CREDIT>$league['VRB']?$league['VRB']:$GSINGLE_CREDIT;
			//$bettop=$league['VRB'];
			$bettop=$GSINGLE_CREDIT;
		}
	}
...
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_hrb_finish.php" method="post" onSubmit="return false">
  <div class="ord ord_top">
    ...
    <div class="main">
      ...
      <div class="gametype">(滚球)&nbsp;让球 - 上半</div>
